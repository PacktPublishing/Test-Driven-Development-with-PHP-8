...
class ToyCarCreatorTest extends KernelTestCase
{
    ...
    public function testCanCreate(ToyCar $toyCarModel): void
    {
        ...
        // Mock 1: Validator
        $validatorStub = $this->createMock(ToyCarValidator::class);
        $validatorStub
            ->method('validate')
            ->willReturn($validationResultStub);

        // Mock 2: Data writer
        $toyWriterStub = $this->createMock(ToyCarWriter::class);
        $toyWriterStub
            ->method('write')
            ->willReturn(true);
     ...
     * @dataProvider provideToyCarModel
     */
    public function testCanThrowCreateException(ToyCar $toyCarModel): void
    {
